...
def load_paid_data():
    """加载或创建数据文件"""
    if os.path.exists(PAID_DATA_FILE):
        return pd.read_csv(PAID_DATA_FILE, parse_dates=['日期'])
    else:
        df = init_data()
        df.to_csv(PAID_DATA_FILE, index=False)
        return df


def save_paid_data(df):
    df.to_csv(PAID_DATA_FILE, index=False)
    st.cache_data.clear()
